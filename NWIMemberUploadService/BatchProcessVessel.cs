    ...
    public class BatchProcessVessel
    {
        ...
        private void WriteLogFile(string text)
        {
            try
            {
                ...
                string path = DirectoryPath + @"\" + DateTime.Now.ToString("dd-MM-yyyy") + ".txt";
        ...
        private void ReadLoadSourceFile()
        {
            //mstrSourceFilePath = ConfigurationSettingReader.VesselFolderPath;
            string[] strFiles = Directory.GetFiles(mstrSourceFilePath);
            WriteLogFile("Before reading Vessel upload file in ReadLoadSourceFile method");
            if (strFiles.Length > 0)
            {
                try
                {
                    //strCurrFileName = strFiles[0];
                    WriteLog("First Method ReadLoadSourceFile for Initial Upload :- " + strCurrFileName + Environment.NewLine + "File Process Started On " + DateTime.Now.Date.ToString("dd/MM/yyyy") + DateTime.Now.Hour.ToString() + ":" + DateTime.Now.Minute.ToString());
                    dtSourceTble = null;
                    ...
                    WriteLogFile("After reading Vessel upload file in ReadLoadSourceFile method");
        ...
        protected void ReadFileAndLoadDataTable(string strFilepath)
        {
            try
            {
                ...
                if (error.Trim().Length > 0)
                {
                    ...
                    WriteLogFile("Unable to read excel file. Error Message:-" + Environment.NewLine + error);
                    return;
                }

                if (ValidateExcel(dtExcel, 24, ref error, "Vessel", "Type", "Built", "GRT", "NRT", "BHP", "Class", "Flag", "Dimension", "Owner", "Managers", "Mortgagees", "InceptionDate", "ExpiryDate", "UnderwriterCode", "SubClassCode", "Currency", "SumInsured", "Rate", "Loading", "Share", "Premium", "Deductible", "Remarks") == false)
                {
                    WriteLog("Excel File format Issue:-" + Environment.NewLine + error);
                    WriteLogFile("Excel File format Issue:-" + Environment.NewLine + error);
                }
                CreateTable();
                string strCurrLine = "";
                ...
                foreach (DataRow dr in dtExcel.Rows)
                {
                    ...
                    if (strArrCurrData != null && (strArrCurrData.Length == 24))
                    {
                        try
                        {
                            intSrNo = intSrNo + 1;
        ...
        private void addInputRowToTable(string[] strArr, string Counter)
        {
            ...
            strTemp = strArr[12].Trim().Replace(",", ".");
            if (strTemp.Trim().Length == 0)
            {
            ...
            strTemp = strArr[13].Trim().Replace(",", ".");
        ...
        private string getFileExtension()
        {
            ...
            try
            {
                ...
                dsDetails = objVesselDetailsManager.GetVesselBatchFileUploadList(objVesselDetails);
        ...
        public static string ReadExcelFile(ref DataTable dtExcel, string filepath)
        {
            ...
            try
            {
                oleExcelCon.ConnectionString = ConfigurationManager.AppSettings["ExcelConnXLSX"].ToString() + filepath;
                oleExcelCon.Open();
                ...
                if (schemaTable.Rows.Count == 1)
                {
                    ...
                    {
                        string strSql = "Select * From [" + schemaTable.Rows[i].ItemArray[2].ToString() + "]";
                    ...
                    error = "Uploaded file should contain only one sheet !";
                oleExcelCon.Close();
            }
            catch (Exception ex)
            {
        ...
        private void UpdateStatus()
        {
            ...
            ds = objVesselDetailsManager.UpdateBatchUploadVessel(objVesselDetails);
        }
        #endregion
    }
}
