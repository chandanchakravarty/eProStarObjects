...
namespace MemberUploadService
{
    public class BatchProcess
    {
        ...
        {
            EventLogs.Publish("Member Upload Service Started", System.Diagnostics.EventLogEntryType.Information);
            ...
            for (i = 1; i < iCount; i++)
            {
                ...
                if (!string.IsNullOrEmpty(mstrConStr.Trim()))
                {
                    ...
                    ReadLoadSourceFile();
                    WriteLogFile("ReadLoadSourceFile");
                   
                    ReadLoadSourceFile_EN();
                    WriteLogFile("ReadLoadSourceFile_EN");
                    
                    ReadLoadSourceFile_MA();
                    WriteLogFile("ReadLoadSourceFile_MA");
                }
            }
        ...
        {
            _strInitialFilePath = ConfigurationSettingReader.InitialFolderPath.ToString();
            ...
            WriteLogFile("ReadLoadSourceFile_1");
            if (strFiles.Length > 0)
            {
                try
                {
                    ...
                    WriteLogFile("ReadLoadSourceFile_2");
                    strRemarks = "";
                    ...
                    WriteLogFile("ReadLoadSourceFile_3");
        ...
        private void ReadLoadSourceFile_EN()
        {
            ...
            _strEnrolledFilePath = ConfigurationSettingReader.EnrolledFolderPath.ToString();
            if (!String.IsNullOrEmpty(_strEnrolledFilePath))
                mstrSourceFilePath = mstrSourceFilePath + "\\" + _strEnrolledFilePath;
            WriteLogFile("ReadLoadSourceFile_EN_1");
            string[] strFiles = Directory.GetFiles(mstrSourceFilePath);
            if (strFiles.Length > 0)
            {
                try
                {
                    ...
                    strUploadType = strCurrFileName.Substring(strCurrFileName.LastIndexOf("\\"), 5).ToString().Trim().Replace("\\", "");
                    strRemarks = "";
                    WriteLogFile("ReadLoadSourceFile_EN_2");
                    if (strUploadType == "MED-")
                    {
                    ...
                    WriteLogFile("ReadLoadSourceFile_EN_3");
                    WriteLog(DateTime.Now.Date.ToString("dd/MM/yyyy") + DateTime.Now.Hour.ToString() + ":" + DateTime.Now.Minute.ToString());
                    WriteLog("File Process completed");
                }
                catch (Exception ex)
        ...
        private void ReadLoadSourceFile_MA()
        {
            ...
            _strEnrolledFilePath = ConfigurationSettingReader.MassAdjustmentFolderPath.ToString();
            if (!String.IsNullOrEmpty(_strEnrolledFilePath))
                mstrSourceFilePath = mstrSourceFilePath + "\\" + _strEnrolledFilePath;
            WriteLogFile("ReadLoadSourceFile_MA_1");
            string[] strFiles = Directory.GetFiles(mstrSourceFilePath);
            if (strFiles.Length > 0)
            {
                try
                {
                    ...
                    strUploadType = strCurrFileName.Substring(strCurrFileName.LastIndexOf("\\"), 5).ToString().Trim().Replace("\\", "");
                    strRemarks = "";
                    WriteLogFile("ReadLoadSourceFile_MA_2");
                    if (strUploadType == "MED-")
                    {
                    ...
                    WriteLogFile("ReadLoadSourceFile_MA_3");
                    WriteLog(DateTime.Now.Date.ToString("dd/MM/yyyy") + DateTime.Now.Hour.ToString() + ":" + DateTime.Now.Minute.ToString());
                    WriteLog("File Process completed");
                }
                catch (Exception ex)
